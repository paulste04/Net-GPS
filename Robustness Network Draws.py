...
all_summaries.to_csv('C:/Users/psfin/Documents/Thesis_Results/network_draws_global_er.csv')

##############

summary_er = pd.read_csv('C:/Users/psfin/Documents/Thesis_Results/network_draws_global_er.csv')
summary_sf = pd.read_csv('C:/Users/psfin/Documents/Thesis_Results/network_draws_global_sf.csv')

# Keep only RMSE metrics
rmse_er = summary_er[summary_er["Metric"].str.contains("RMSE")]
rmse_er["Network"] = "Erdős–Rényi"

rmse_sf = summary_sf[summary_sf["Metric"].str.contains("RMSE")]
rmse_sf["Network"] = "Scale-Free"

# Combine
rmse_all = pd.concat([rmse_er, rmse_sf], ignore_index=True)

# Extract Estimator from Metric column
rmse_all["Estimator"] = rmse_all["Metric"].apply(lambda x: "Naive" if "Naive" in x else "Net-GPS")

# Create a clean label for normalization type (row/eigen)
rmse_all["Normalization"] = rmse_all["Metric"].apply(lambda x: "Row" if "row" in x else "Eigen")


import seaborn as sns

rmse_all["Net_Norm"] = rmse_all["Network"] + " (" + rmse_all["Normalization"] + ")"
...
#plt.title("RMSE Distribution by Estimator and Network Type")
plt.ylabel("RMSE",fontsize=14)
plt.xlabel("Estimator", fontsize=14)
plt.xticks(fontsize=14)
...
# Jetzt pivot, damit beide Estimatoren in einer Zeile stehen
pivoted = rmse_all.pivot_table(
    index=["net_draw", "Network", "Normalization"],
    columns="Estimator",
    values="Point"
).reset_index()

# Rechne Differenz
pivoted["RMSE_Diff"] = pivoted["Net-GPS"] - pivoted["Naive"]
...
plt.title("RMSE(Net-GPS) − RMSE(Naive) across Network Draws")
...
plt.fill_between(t_vals, lo_grad_row, hi_grad_row, alpha=.15)
